# Graph Report - estehh  (2026-09-20)

## Corpus Check
- 78 files · ~72,255 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 500 nodes · 713 edges · 45 communities (36 shown, 9 thin omitted)
...
- `extractTimestamp()` --calls--> `parseTimestamp()`  [EXTRACTED]
...
- `KasKeluarPanel()` --calls--> `parseTimestamp()`  [EXTRACTED]
  frontend/src/components/dashboard/KasKeluarPanel.jsx → frontend/src/utils/formatters.js
...
Cohesion: 0.10
Nodes (29): getDefaultFilter(), PAGE_SIZE_OPTIONS, ReportPanel(), applyReportFilters(), buildDenominationList(), buildReportStats(), buildReportTableData(), getField() (+21 more)

### Community 1 - "DashboardPage.jsx"
Cohesion: 0.08
Nodes (22): ConfirmDialog(), AddCabangModal(), AddKaryawanModal(), CabangPanel(), normalizeCabang(), getInitials(), KaryawanPanel(), normalizeKaryawan() (+14 more)
...
Cohesion: 0.12
Nodes (34): BarChart(), buildSegments(), DonutChart(), buildPoints(), LineChart(), getDefaultFilter(), OverviewPanel(), ReportTable() (+26 more)

### Community 5 - "Postman Unit Test - Gas Backend API"
Cohesion: 0.04
Nodes (47): 10) Negative Test `read_master` pakai token staff (harus gagal), 10a) Negative Test `setup_rekap` pakai token staff (harus gagal), 11) Negative Test `create` tanpa token, 12) Test Endpoint `logout` (admin), 13) Negative Test token revoked (setelah logout), 14) Opsional - Jalankan otomatis via Collection Runner, 1) Persiapan, 2) Struktur Collection (+39 more)
...
Cohesion: 0.21
Nodes (22): appendTableRow_(), applyMasterDataValidations_(), columnIndexToLetter_(), ensureMonthlyTabsExist_(), formatHeaderRange_(), getBahanHeaderPrefix_(), getFieldCaseInsensitive_(), getMasterSheet_() (+14 more)
...
Cohesion: 0.50
Nodes (7): getCachedSession_(), getSessionCache_(), handleLogin_(), handleLogout_(), putSessionCache_(), removeSessionCache_(), requireAuthSession_()

### Community 26 - "TransactionService.js"
Cohesion: 0.43
Nodes (5): getPreviousPeriod_(), handleCreateReport_(), handleGetInitialFormData_(), handleUpdateReport_(), syncReportExpenses_()
...
Cohesion: 0.60
Nodes (4): handleUpdateMaster_(), onEdit(), syncBahanBakuToTipePengeluaran_(), syncCabangToSumberPemasukan_()
...
- **Why does `parseTimestamp()` connect `dashboard.js` to `reports.js`, `DashboardPage.jsx`, `KasKeluarPanel.jsx`?**
  _High betweenness centrality (0.008) - this node is a cross-community bridge._
